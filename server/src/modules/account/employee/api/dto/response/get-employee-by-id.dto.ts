import { AccountDocument } from "@Modules/account/account/data";
import { DepartmentDocument } from "@Modules/department/data/department.schema";
import { PositionDocument } from "@Modules/position/data/position.schema";
import { TechnologyDocument } from "@Modules/technology/data/technology.schema";
import { parsImageUrl } from "@Package/file";

export function getEmployeeByIdDto(account: AccountDocument) {
    return {
        id: account._id,
        email: account.email,
        firstName: account.firstName,
        lastName: account.lastName,
        phoneNumber: account.phoneNumber,
        department: (account.employee?.department as any)?.name || 'Unknown',
        position: (account.employee?.position as any)?.name || 'Unknown',
        technologies: (account.employee?.technologies as any[])?.map((technology) => technology?.name) || [],
        employmentType: account.employee?.employmentType,
        baseSalary: account.employee?.baseSalary,
        image: account.image ? parsImageUrl(account.image) : null,
        status: account.employee?.status ?? null,
        hireDate: account.employee?.hireDate ?? null,
        birthday: account.birthday ?? null,
    }
}