...
interface UseModalStateReturn<T = any> {
  ...
  openApproveModal: (data: T) => void
  openProcessModal: (data: T) => void
  closeModal: () => void
...
export const useModalState = <T = any>(): UseModalStateReturn<T> => {
  ...
  const openApproveModal = useCallback((data: T) => {
    setModalState({
      isOpen: true,
      mode: 'approve',
      data
  ...
  const openProcessModal = useCallback((data: T) => {
    setModalState({
      isOpen: true,
      mode: 'process',
      data
  ...
  const isApproveMode = modalState.mode === 'approve'
  const isProcessMode = modalState.mode === 'process'
